        ...
        if (! empty($product) && $product->enable_stock == 1) {
            ...
            foreach ($transactions as $transaction) {
                foreach ($transaction->purchase_lines as $purchase_line) {
                    ...
                    $purchase_lines[$purchase_line->variation_id][$k]['created_by'] = $transaction->created_by;
                }
            }
    ...
    public function save(Request $request)
    {
        ...
        try {
            ...
            if (! empty($product) && $product->enable_stock == 1) {
                ...
                $existing_purchase_lines = PurchaseLine::where('product_id', $product->id)
                ->pluck('id', 'transaction_id')->toArray();

                $existing_transactions = Transaction::where('type', 'opening_stock')
                        ...
                        if (array_key_exists($location_id, $locations)) {
                            foreach ($value as $vid => $purchase_lines_data) {
                                ...
                                foreach ($purchase_lines_data as $k => $pl) {
                                    ...
                                    $processed_transaction_ids = array_merge($processed_transaction_ids, array_keys($edit_purchase_lines));
                                }
                            }
                ...
                if (empty($processed_transaction_ids)) {
                    $transactions_to_delete = $existing_transactions;
                } else {
                    $transactions_to_delete = array_diff($existing_transactions, $processed_transaction_ids);
                }
